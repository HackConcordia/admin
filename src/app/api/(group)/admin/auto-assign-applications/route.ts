...
/**
 * Auto-assigns unassigned "Submitted" applications to reviewers (non-SuperAdmin admins)
...
export const POST = async (req: NextRequest) => {
  try {
    ...
    if (!payload.isSuperAdmin) {
      return sendErrorResponse("Forbidden: Only SuperAdmins can auto-assign applications", null, 403);
    }
    ...
    console.log(`Found ${unassignedApplications.length} unassigned applications`);
    ...
    const reviewers = await Admin.find({ isSuperAdmin: false }).select("email assignedApplications").lean().exec();

    if (!reviewers || reviewers.length === 0) {
      return sendErrorResponse("No reviewers available. Cannot auto-assign applications.", null, 400);
    }
    ...
    const sortedGroups = Array.from(teamGroups.entries()).sort((a, b) => b[1].length - a[1].length);
    ...
    for (const [teamId, applicationIds] of sortedGroups) {
      ...
      let minReviewer: { email: string; count: number; applications: string[] } | null = null;
    ...
    console.log(`Successfully assigned ${unassignedApplications.length} applications`);

    // Prepare response statistics
    const reviewerStats = Array.from(assignmentsByReviewer.entries()).map(([email, apps]) => ({
      reviewer: email,
